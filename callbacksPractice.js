...
function intersection(arrays) {
  return arrays.reduce((accumulator, current) => {
    	return current.filter(element => accumulator.includes(element))
  	});
}
...
function union(arrays) {
	return arrays.reduce((accumulator, current) => {
    const elements = current.filter(element => !accumulator.includes(element));
...
function multiMap(arrVals, arrCallbacks) {
  ...
  arrVals.map((arr) => {
    object[arr] = [];
    object[arr] = (arrCallbacks.map(callback => callback(arr)))
  })
  return object
}
...
function majority(array, callback) {
  ...
  array.map(el => callback(el) ? countTrue++ : countFalse++)
...
function prioritize(array, callback) {
  const output = []
  const falseArr = []
  array.map(el => callback(el) ? output.push(el) : falseArr.push(el))
  return output.concat(falseArr)
}
